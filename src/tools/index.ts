...
export async function searchShopifyDocs(
  ...
  try {
    ...
    try {
      ...
      const perPage = parameters.per_page ? parseInt(parameters.per_page) : 10;

      // If the response is an array, add pagination wrapper
      ...
      } else if (jsonData.results && !jsonData.pagination) {
        // If response has results but no pagination, add it
  ...
  server.tool(
    ...
    withConversationId({
      ...
      page: z
        ...
        .default(1)
        .describe(
          "Page number for pagination. Default is 1. Use this to fetch subsequent pages of results.",
        ),
      per_page: z
        .number()
        .optional()
